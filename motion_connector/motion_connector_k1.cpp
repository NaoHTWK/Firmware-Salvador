...
void MotionConnector::proceed() {
    ...
    if ((gc_state.state == GameState::Initial || gc_state.state == GameState::Finished ||
        ...
        if (gc_state.state != GameState::Set) {
            head_angles = {0.f, 0.f};
            int64_t standing_time = time_us() - start_standing_time;
            if (standing_time > 5000000) {
                if (current_mode != booster::robot::RobotMode::kPrepare)
                    // client.ChangeMode(booster::robot::RobotMode::kPrepare);
                    // current_mode = booster::robot::RobotMode::kPrepare;
                    standing_time = 0;
                return;
        ...
        send_high_level_motion({0.f, 0.f, 0.f}, head_angles);
        return;
    }
    standing = false;
    ...
    if (motion_command.type == MotionCommand::Type::JOINT_CONTROL) {
        ...
        if (current_mode != booster::robot::RobotMode::kWalking) {
            if (!gc_state.is_fake_gc) {
                // client.ChangeMode(booster::robot::RobotMode::kWalking);
                current_mode = booster::robot::RobotMode::kWalking;
            }
            LOG_F(INFO, "Changing mode to kWalking (WALK)");
            current_mode = booster::robot::RobotMode::kWalking;
        }
        send_high_level_motion(motion_command.walk_request, head_angles);
    } else if (motion_command.type == MotionCommand::Type::STAND ||
               motion_command.type == MotionCommand::Type::NOTHING) {
        if (current_mode != booster::robot::RobotMode::kWalking && !gc_state.is_fake_gc) {
            LOG_F(INFO, "Changing mode to kWalking (STAND/NOTHING)");
            client.ChangeMode(booster::robot::RobotMode::kWalking);
            current_mode = booster::robot::RobotMode::kWalking;
        }
        send_high_level_motion({0.f, 0.f, 0.f}, head_angles);
