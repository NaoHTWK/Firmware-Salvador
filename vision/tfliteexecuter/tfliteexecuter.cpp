...
void TFLiteExecuter::loadModelFromFile(const std::string& file, std::vector<int> inputDims, int numThreads, TfLiteType input_type, TfLiteType output_type) {
    ...
    MY_ASSERT_EQ(TfLiteTensorType(inputTensor), input_type);

    const TfLiteTensor* outputTensor = TfLiteInterpreterGetOutputTensor(interpreter, 0);
    MY_ASSERT_NE(outputTensor, nullptr);
    MY_ASSERT_EQ(TfLiteTensorType(outputTensor), output_type);
}
...
std::string TFLiteExecuter::getTFliteModelPath() {
    ...
    if (val != nullptr) {
        ...
        tflitePath = "/home/booster/etc/tflite";
