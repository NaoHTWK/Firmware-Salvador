...
vector<LineGroup> LineDetector::getLineGroups() {
    vector<LineGroup> scaledLines = {};
    for (const LineGroup &lg : linesList) {
        scaledLines.push_back(lg);
    }
    return scaledLines;
}
 ...
 */
void LineDetector::proceed(Image* img, vector<LineSegment *> lineSegments, int q) {
    ...
    // search for near line-edges with similar angle and group/link them together into individual
    // neighborhood lists for every line-edge
    rMax = (int)(q * 8);
    rMin = (int)(q * 0.9f);
    for (LineSegment *ls : lineSegments) {
        LineSegment *neighbor = ls->pred;
    ...
    // search points, which only have neighbors in one direction (possible end-points from long
    // lines in the image)
    int angles[36];
    ...
    // for every end-point, try to find a line with as much as possible line-edges on it by
    // traveling step by step through the connected neighborhood
    int id = 0;
    lineSegments.clear();
    ...
    lines_channel.publish(getLineGroups());
}

LineEdge LineDetector::createLineEdge(const std::vector<LineSegment *> &segments) {
    float avgNx = 0;
    float avgNy = 0;
    float avgXM = 0;
    float avgYM = 0;
...
optional<point_2d> LineDetector::getIntersection(float px1, float py1, float vx1, float vy1,
...
void LineDetector::updateWhiteColor(const std::vector<LineSegment *> &lineSegments, Image* img) {
    ...
    for (LineEdge *le : lineEdges) {
        ...
        if (le->id == -1 || (le->id < id && le->matchCnt < minConnectionCnt)) {
            ...
            for (LineSegment *ls : le->segments) {
                ...
                if (neighbor->id == -1 ||
                    (neighbor->id < id && neighbor->matchCnt < minConnectionCnt)) {
                    neighbor->id = id;
                    neighbor->matchCnt = 1;
                }
            }
