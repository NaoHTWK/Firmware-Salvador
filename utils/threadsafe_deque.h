...
public:
    ThreadSafeDeque(size_t max_size = 0) : max_size(max_size), shutdown_flag(false) {}
    ...
    T pop_front() {
        ...
        cv.wait(lock, [this]() { return !storage.empty() || shutdown_flag; });

        if (shutdown_flag && storage.empty()) {
            // Return default-constructed T if shutting down
            return T();
        }
