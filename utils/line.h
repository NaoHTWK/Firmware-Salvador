...
struct Line {
    ...
    // Returns > 0 if the point is left of the line, == 0 if it's on the line, and < 0 if it's right
    // of the line. If you have a left-handed coordinate system (like the image coordinates) you'll
    ...
    std::optional<point_2d> intersect_within(const Line& other) const {
        if (auto p = intersect(other))
            return orthogonalProjection(*p).inside && other.orthogonalProjection(*p).inside
                           ? p
                           : std::nullopt;
        return std::nullopt;
    }
    ...
    std::string toString() const {
        return "[" + std::to_string(px1) + "," + std::to_string(py1) + "]->[" +
               std::to_string(px2) + "," + std::to_string(py2) + "]";
    }
