...
public:
    ...
    ThreadPool(const std::string& name, bool low_prio = false, uint32_t num_threads = 0) : shutdown_flag(false) {
        if (num_threads == 0) {
        ...
        for (uint32_t i = 0; i < num_threads; i++) {
            threads.emplace_back(launch_named_thread(name + "_" + std::to_string(i), low_prio, [this]() {
                while (!shutdown_flag) {
                    auto task = tasks.pop_front();
                    // Check if we got a valid task (not a shutdown signal)
                    if (!shutdown_flag) {
                        task();
                    }
                }
    ...
    ~ThreadPool() {
        ...
        // Signal the tasks queue to shutdown (wakes up waiting threads)
        tasks.shutdown();
