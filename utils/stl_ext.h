...
/**
 * @brief Returns lower_val if p < lower_bound, upper_val if p > upper_bound, linear interpolation
 * otherwise.
 * @attention Requires lower_bound <= upper_bound.
 * @example You have a speed (speed_close) at which the ball should be approached when you're close
 * to the ball and another speed (speed_far) at which the ball should be approached when you're far
 * away. Let's say that when you're closer than 0.5m speed_close should be used and when you're
 * farther than 1m speed_far. If the ball is between 0.5-1m you want a smooth transition between the
 * 2 speeds. You can now just run clamped_linear_interpolation(distance_to_ball, speed_close,
 * speed_far, 0.5f, 1.f) and get exactly the speed you want.
 */
template <typename T>
T clamped_linear_interpolation(float p, const T& lower_val, const T& upper_val, float lower_bound,
                               float upper_bound) {
    if (p < lower_bound)
        return lower_val;
    if (p > upper_bound)
...
template <typename Map>
std::optional<const typename Map::mapped_type> find(const Map& map,
...
constexpr int round_int(F f) {
    return static_cast<int>(f >= static_cast<F>(0) ? f + static_cast<F>(0.5)
                                                   : f - static_cast<F>(0.5));
}
...
[[nodiscard]] std::thread launch_named_thread(const std::string& name, bool low_prio, Callable&& callable, Args&&... args) {
    return std::thread([name, callable = std::forward<Callable>(callable), ... args = std::forward<Args>(args)]() {
