...
#define sort_desc(cont, elem)                                                                \
    std::sort((cont).begin(), (cont).end(),                                                  \
              [](const decltype(cont)::value_type& a, const decltype(cont)::value_type& b) { \
...
auto min_element_transformed(Container& c,
                             std::function<float(const typename Container::value_type&)> trans) {
    return std::min_element(
            c.begin(), c.end(),
            [&trans](const typename Container::value_type& a,
                     const typename Container::value_type& b) { return trans(a) < trans(b); });
}
...
void insert_if(Container& in, ContainerOut& out, UnaryPredicate pred) {
    if constexpr (std::is_same<ContainerOut,
                               std::vector<typename ContainerOut::value_type,
                                           typename ContainerOut::allocator_type>>::value ||
                  std::is_same<ContainerOut,
                               std::deque<typename ContainerOut::value_type,
                                          typename ContainerOut::allocator_type>>::value ||
                  std::is_same<ContainerOut,
                               std::list<typename ContainerOut::value_type,
                                         typename ContainerOut::allocator_type>>::value) {
...
void erase_if(Container& c, UnaryPredicate pred) {
    ...
    } else if constexpr (std::is_same<Container,
...
template <class Container, class UnaryPredicate>
auto find_with_default(const Container& c, UnaryPredicate pred,
